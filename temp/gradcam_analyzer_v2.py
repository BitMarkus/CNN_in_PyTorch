...
class GradCAMAnalyzer:
    ...
    def generate_heatmap(self, input_tensor, target_class=None):
        ...
        # If no target_class specified, use the predicted class
        if target_class is None:
            target_class = predicted_class
        else:
            # Verify the provided target_class matches prediction
            if predicted_class != target_class:
                print(f"Warning: Image predicted as {self.classes[predicted_class]} but analyzing for {self.classes[target_class]}")
    ...
    def __call__(self):
        ...
        for i, (img, label) in enumerate(self.ds.ds_pred):
            img_path = Path(self.ds.ds_pred.dataset.samples[i][0])
            print(f"Image: {img_path.name} | Folder: {img_path.parent.name} | Label: {label} ({self.classes[label]})")
