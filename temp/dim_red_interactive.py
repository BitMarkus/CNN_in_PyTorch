...
try:
    ...
    print("Plotly not available. Interactive plots disabled. Install with: pip install plotly")
...
class DimRed:
    ...
    def __init__(self, device):
        ...
        # NEW: Interactive plot settings
        self.create_interactive_plots = setting.get('dimred_create_interactive', False)
        ...
        # Print interactive plot status
        if self.create_interactive_plots:
            if PLOTLY_AVAILABLE:
                print(f"Interactive plots: ENABLED (sample size: {self.interactive_sample_size})")
            else:
                print("Interactive plots: DISABLED (Plotly not installed)")
                self.create_interactive_plots = False
        else:
            print("Interactive plots: DISABLED (setting: dimred_create_interactive = False)")
    ...
    def create_interactive_plot(self, embeddings, labels, method_name):
        """
        Create interactive Plotly visualization with optional image preview on hover
        """
        if not PLOTLY_AVAILABLE:
            print("Plotly not available. Skipping interactive plot.")
            return None
        
        if not self.create_interactive_plots:
            print("Interactive plots disabled in settings.")
            return None
        
        print(f"Creating interactive {method_name} plot...")
        
        # Sample data for performance
        n_samples = min(self.interactive_sample_size, len(embeddings))
        if n_samples < len(embeddings):
            indices = np.random.choice(len(embeddings), n_samples, replace=False)
            sampled_embeddings = embeddings[indices]
            sampled_labels = labels[indices]
            print(f"Sampled {n_samples} points for interactive plot (from {len(embeddings)} total)")
        else:
        ...
        if self.mode == "groups":
            display_names = []
            for label_val in sampled_labels:
                found = False
            ...
            display_names = [self.classes[label] for label in sampled_labels]
        ...
        df = pd.DataFrame({
            ...
            'point_id': range(len(sampled_embeddings))
        })
        
        # Add image paths for groups mode if available
        if self.mode == "groups" and hasattr(self, 'ds') and hasattr(self.ds, 'ds_pred'):
            try:
                all_image_paths = [self.ds.ds_pred.dataset.samples[i][0] for i in range(len(labels))]
                sampled_image_paths = [all_image_paths[i] for i in indices]
                df['image_path'] = sampled_image_paths
                
                # Generate thumbnails
                def image_to_base64(image_path, size=(self.interactive_thumbnail_size, self.interactive_thumbnail_size)):
                    try:
                ...
                df['thumbnail'] = df['image_path'].apply(image_to_base64)
                
                # Create plot with images
                fig = px.scatter(df, 
                                x='x', y='y',
                                color='display_name',
                                hover_data={'thumbnail': True, 'display_name': True, 'image_path': True},
                                title=f'{method_name} - Interactive Visualization<br><sub>Hover to see images</sub>')
                
                fig.update_traces(
                    hovertemplate='''
                    <b>%{customdata[1]}</b><br>
                    <img src="%{customdata[0]}" width="{size}" height="{size}"><br>
                    <extra></extra>
                    '''.format(size=self.interactive_thumbnail_size),
                    marker=dict(size=8, opacity=0.7)
                )
                
            except Exception as e:
                print(f"Error loading images for interactive plot: {e}")
                # Fall back to basic plot without images
                fig = px.scatter(df, 
                                x='x', y='y',
                                color='display_name',
                                title=f'{method_name} - Interactive Visualization')
        else:
            # Basic plot without images
            fig = px.scatter(df, 
                            x='x', y='y',
                            color='display_name',
                            title=f'{method_name} - Interactive Visualization')
        ...
        if self.mode == "groups":
            output_path = output_dir / f"{method_name.lower()}_interactive_{len(self.group_mapping)}_groups.html"
        else:
            output_path = output_dir / f"{method_name.lower()}_interactive_{self.mode}.html"
        ...
        try:
            ...
            print(f"Note: Interactive plot saved to file. Open {output_path} in a web browser to view.")
    ...
    def run_reduction(self, method, reducer, features, labels):
        ...
        if self.create_interactive_plots and PLOTLY_AVAILABLE:
            self.create_interactive_plot(embedding, labels, method)
        
        # Create static matplotlib plot (original functionality)
        ...
        plt.close()
        print(f"Saved {method} static plot to {output_path}")
...
# Interactive plot settings (optional)
'dimred_create_interactive': False,  # Set to True to enable interactive plots
'dimred_interactive_sample_size': 2000,  # Number of points to show in interactive plot
'dimred_interactive_thumbnail_size': 80,  # Thumbnail size in pixels
"""