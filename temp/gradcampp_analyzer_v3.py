...
class GradCAMpp_Analyzer:
    ...
    def generate_gradcam(self, input_tensor, target_class):
        """Original GradCAM implementation from old script"""
        ...
        try:
            ...
            with torch.no_grad():
                if hasattr(self.cnn.model, 'features'):
                    ...
                    # Fallback for models without features attribute
                    features = None
                    for name, module in self.cnn.model.named_children():
                        if name == 'conv_layers' or name == 'features':
                            features = module(input_tensor.unsqueeze(0))
                            break
                    if features is None:
                        raise ValueError("Could not find feature extraction layers")
                
                activations = features.detach().requires_grad_(True)
            
            # Get model output
            if hasattr(self.cnn.model, 'classifier'):
                output = self.cnn.model.classifier(F.relu(F.adaptive_avg_pool2d(activations, (1, 1))).view(1, -1))
            else:
                # Fallback for models without classifier attribute
