...
class GradCAMpp_Analyzer:
    ...
    def __init__(self, device: torch.device, debug: bool = False):
        ...
        try:
            if hasattr(self.cnn.model, 'features'):
                self.gradcam_target_layer = self.cnn.model.features.denseblock4.denselayer16.conv2
            elif hasattr(self.cnn.model, 'layer4'):
                self.gradcam_target_layer = self.cnn.model.layer4[-1].conv2
            else:
                for name, module in self.cnn.model.named_modules():
                    if isinstance(module, torch.nn.Conv2d):
                        self.gradcam_target_layer = module
                if not hasattr(self, 'gradcam_target_layer'):
                    raise ValueError("Could not find suitable convolutional layer")
            
            if debug:
                print(f"Selected target layer: {self.gradcam_target_layer}")
                
        except AttributeError as e:
            raise ValueError(f"Could not automatically determine target layer: {str(e)}")
    ...
    def _prepare_image(self, input_tensor: torch.Tensor) -> np.ndarray:
        ...
        img_np = input_tensor.squeeze().detach().cpu().numpy()  # Added detach()
    ...
    def ig_analysis(self, input_tensor: torch.Tensor, target_class: int) -> np.ndarray:
        """IG implementation with consistent positive-only output"""
        try:
            torch.cuda.empty_cache()
            attr_np = np.zeros((input_tensor.shape[2], input_tensor.shape[3]))
            ...
            with torch.no_grad():
                ...
                try:
                    ...
                    print(f"IG attribution failed: {str(e)}")
                    return attr_np
                
                if attr_np.ndim == 3:
                    attr_np = np.mean(attr_np, axis=0)
                    
                # Only positive attributions with enhanced visibility
                pos_attr = np.maximum(attr_np, 0)
                pos_attr = gaussian_filter(pos_attr, sigma=2.0)
                
                if np.max(pos_attr) > 0:
                    # Apply power law scaling for better visibility
            ...
            print(f"IG analysis failed completely: {str(e)}")
            return np.zeros((input_tensor.shape[2], input_tensor.shape[3]))
        finally:
            torch.cuda.empty_cache()

    def visualize(self, input_tensor, label, img_path):
        """Handle complete visualization pipeline"""
        try:
            ...
            gradcam_map = np.zeros(input_tensor.shape[2:])
            ig_map = np.zeros(input_tensor.shape[2:])
            
            try:
                gradcam_map = self.gradcam_pp(input_tensor.clone(), label.item())
            except Exception as e:
                print(f"GradCAM failed: {str(e)}")
                
            try:
                ig_map = self.ig_analysis(input_tensor.clone(), label.item())
            except Exception as e:
                print(f"IG failed: {str(e)}")
            
            try:
                img_np = self._prepare_image(input_tensor)
