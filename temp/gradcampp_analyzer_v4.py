...
class GradCAMpp_Analyzer:
    ...
    def gradcam_pp_improved(self, input_tensor: torch.Tensor, target_class: int) -> np.ndarray:
        """Enhanced GradCAM++ implementation with smoother output"""
        try:
    ...
    def visualize(self, input_tensor, img_path, target_class):
        ...
        try:
            ...
            ####################################################################################################################################################
            # gradcam_map = self.gradcam_pp(input_tensor[0].clone(), target_class)
    ...
    def _create_visualization(self, img_np_plot, gradcam_map, ig_map, target_class, pred_prob, img_path):
        """Create visualization matching original GradCAM style"""
        plt.ioff()
        try:
            ...
            # IG Heatmap (coolwarm for contrast)
            ax3 = fig.add_subplot(gs[1, 0])
            im3 = ax3.imshow(ig_heat, cmap='coolwarm')
            ...
            ax4.imshow(img_plot if self.img_channels == 1 else np.mean(img_plot, axis=2),
                    cmap='gray' if self.img_channels == 1 else None)
            ax4.imshow(ig_heat, cmap='coolwarm', alpha=self.alpha_overlay)
            ...
            ax5.imshow(ig_heat, cmap='coolwarm', alpha=self.alpha_overlay/2)
