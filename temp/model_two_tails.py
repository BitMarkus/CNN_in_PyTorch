...
class CNN_Model():
    ...
    def load_model(self, device):
        ...
        model_dict = {
            ...
            # NEW: Dual-head DenseNet121 for two-stage learning
            "densenet121_dual": self._build_densenet121_dual
        }
        
        # NEW DUAL-HEAD DENSENET CASE
        if self.cnn_type == "densenet121_dual":
            self.is_pretrained = setting.get("cnn_is_pretrained", False)  # Can be pretrained or not
            self.model = self._build_densenet121_dual()
            
        # Custom Model Case
        elif self.cnn_type == "custom":
        ...
        # Predefined Model Case (original behavior)
        else:
            ...
            # Common Modifications for single-head models
        ...
        # Initialize non-pretrained weights (for all models)
    ...
    # NEW: Build dual-head DenseNet121 for two-stage learning
    def _build_densenet121_dual(self):
        """
        Creates a DenseNet121 with TWO classification heads:
        1. Head 1: WT/KO classification (2 classes) - Easy task
        2. Head 2: Cell line classification (9 classes) - Hard task
        
        Shared backbone features, separate final layers for transfer learning experiments.
        """
        ...
        if self.input_channels == 1:
            original_conv = self._adapt_first_conv(base_model, self.input_channels)
            if self.is_pretrained and original_conv:
                # Transfer pretrained weights by averaging RGB channels
                if hasattr(base_model.features, 'conv0'):
                    base_model.features.conv0.weight.data = original_conv.weight.data.mean(dim=1, keepdim=True)
        
        # Extract the features and classifier
        features = base_model.features
        in_features = base_model.classifier.in_features
        
        # Create the dual-head model
        class DualHeadDenseNet121(nn.Module):
            def __init__(self, features, in_features, num_classes_primary=2, num_classes_secondary=9):
                super().__init__()
                self.features = features
                
                # WT/KO head (primary - easy task)
                self.classifier_primary = nn.Sequential(
                    nn.AdaptiveAvgPool2d((1, 1)),
                    nn.Flatten(),
                    nn.Linear(in_features, 512),
                    nn.ReLU(inplace=True),
                    nn.Dropout(0.5),
                    nn.Linear(512, num_classes_primary)
                )
                
                # Cell line head (secondary - hard task)
                self.classifier_secondary = nn.Sequential(
                    nn.AdaptiveAvgPool2d((1, 1)),
                    nn.Flatten(),
                    ...
                    nn.ReLU(inplace=True),
                    nn.Dropout(0.5),
                    nn.Linear(512, num_classes_secondary)
                )
                
                # Freezing options (can be controlled during training)
                self.freeze_features = False
            ...
            def forward(self, x, task='both'):
                ...
                Args:
                    x: input tensor
                    task: 'primary' (WT/KO), 'secondary' (cell lines), or 'both'
                """
                features = self.features(x)
                
                if task == 'primary':
                    return self.classifier_primary(features)
                elif task == 'secondary':
                    return self.classifier_secondary(features)
                else:  # 'both'
                    return {
                        'primary': self.classifier_primary(features),
                        'secondary': self.classifier_secondary(features)
                    }
            
            def freeze_backbone(self, freeze=True):
                """Freeze/unfreeze the feature extractor"""
                for param in self.features.parameters():
                    param.requires_grad = not freeze
                self.freeze_features = freeze
                
            def freeze_primary_head(self, freeze=True):
                """Freeze/unfreeze the primary (WT/KO) head"""
                for param in self.classifier_primary.parameters():
                    param.requires_grad = not freeze
                self.freeze_primary = freeze
                
            def get_trainable_params(self):
                """Get counts of trainable parameters"""
                total_params = sum(p.numel() for p in self.parameters())
                trainable_params = sum(p.numel() for p in self.parameters() if p.requires_grad)
                return total_params, trainable_params
        
        # Determine number of classes for each task
        # Assuming your classes list contains all 9 cell lines
        # WT/KO task: always 2 classes
        # Cell line task: all 9 classes
        num_primary_classes = 2  # WT vs KO
        num_secondary_classes = self.num_classes  # All cell lines
        
        # Create the dual-head model
        dual_model = DualHeadDenseNet121(
            features=features,
            in_features=in_features,
            num_classes_primary=num_primary_classes,
            num_classes_secondary=num_secondary_classes
        )
    ...
    # Function for Prediction - UPDATED FOR DUAL-HEAD MODEL
    def predict(self, dataset, task='both'):
        ...
        # For dual-head model, we need to handle task selection
        is_dual_head = self.cnn_type == "densenet121_dual"
        ...
        with torch.no_grad():
            ...
            for i, (images, labels) in enumerate(tqdm(dataset)):
                ...
                # Forward pass
                if is_dual_head:
                    # For dual-head model, we can specify which task
                    if task == 'primary':
                        scores = self.model(images, task='primary')
                    elif task == 'secondary':
                        scores = self.model(images, task='secondary')
                    else:
                        # For 'both', we need to decide which head to use for prediction
                        # Default to secondary (cell lines) if not specified
                        scores = self.model(images, task='secondary')
                else:
